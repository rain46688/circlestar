...
/**
 * Servlet implementation class TradeOnGoingPageServlet
 */
@WebServlet("/member/onGoingList")
public class TradeOnGoingPageServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TradeOnGoingPageServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(loginnedMember.getUsid()==usid) {
			...
			List<Card> blist=new MemberService().onGoingList(cPage, numPerPage, usid);
			...
			int totalData=new MemberService().onGoingCount(usid);
			...
			if(pageNo == 1) {
				...
				pageBar += "<li class='page-item'><a class='page-link' href='"+ request.getContextPath() + "/member/onGoingList?usid="+usid+"&cPage=" + (pageNo-1) + "'>이전</a></li>";
			}
			...
			while(pageNo <= pageEnd && pageNo <= totalPage) {
				if(cPage == pageNo) {
					...
					"/member/onGoingList?usid="+usid+"&cPage=" + pageNo +"')>"+pageNo+"</a></li>";
				}
				pageNo++;
			}
			...
			if(pageNo > totalPage) {
				...
				pageBar += "<li class='page-item'><a class='page-link' href='" + request.getContextPath() + "/member/onGoingList?usid="+usid+"&cPage=" + pageNo + "'>다음</a></li>";
			}
