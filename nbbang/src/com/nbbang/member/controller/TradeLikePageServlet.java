...
/**
 * Servlet implementation class TradeLikePageServlet
 */
@WebServlet("/member/likeList")
public class TradeLikePageServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TradeLikePageServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Card> blist=new MemberService().likeList(cPage, numPerPage, usid);
		...
		int totalData=new MemberService().likeCount(usid);
		...
		if(pageNo == 1) {
			...
			pageBar += "<li class='page-item'><a class='page-link' href='"+ request.getContextPath() + "/member/likeList?usid="+usid+"&cPage=" + (pageNo-1) + "'>이전</a></li>";
		}
		...
		while(pageNo <= pageEnd && pageNo <= totalPage) {
			if(cPage == pageNo) {
				...
				"/member/likeList?usid="+usid+"&cPage=" + pageNo +"')>"+pageNo+"</a></li>";
			}
			pageNo++;
		}
		...
		if(pageNo > totalPage) {
			...
			pageBar += "<li class='page-item'><a class='page-link' href='" + request.getContextPath() + "/member/likeList?usid="+usid+"&cPage=" + pageNo + "'>다음</a></li>";
		}
